...
module.exports = function (cloudant, dbName, resourcePath, databaseStateMap, logger) {
  ...
  return {
    updateDocs(allDocs) { // TODO change all docs to object
  ...
  function updateDocsBulkFromDir(docsDir, design) {
    ...
    const createBulkPayload = files => {
      const docs = files.reduce((acc, f) => {
        ...
        try {
          ...
          logger.error(`Failed to parse (skipping) '${docPath}: ${e.message}`, e);
          return acc;
        }
        if (design) {
          if (doc._rev) {
            logger.error(`Couchinator removed '_rev' property from ${docPath}. Will still update design doc if necessary.`)
            delete doc._rev;
          }
          acc.push(doc);
        } else {
          if (doc.docs) {
            ...
            logger.error(`Malformed 'docs' property required (skipping) '${docPath}: Use bulk document format.`)
          }
        }
    ...
    return fsu.files(docsDir)
      .then(createBulkPayload)
      .then(payload => design ? upsert(payload) : payload)
      .then(r => bulkInsert(r))
      .catch(e => {
        if (e.reason) {
          self.logger.error(`Failed to insert documents ${e.reason || e.message}`, e);
        } else {
          throw e;
        }
      });
