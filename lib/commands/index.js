...
function Commands(cloudant, resourcePath, visitor) {
  ...
  return {
    create(opts = { designDocsOnly: false }) {
      ...
      const createDbsAndUpdateDocs = dbs => Promise.all(
        dbs.map(db => createDb(db)
          .then(() => DocUpdater(cloudant, db, resourcePath, databaseStateMap, logger)
            .updateDocs(allDocs))));

      return fsu.directories(resourcePath)
        .then(createDbsAndUpdateDocs)
        .then(() => logger.info('Done.'))
        .catch(e => logger.error(`Database create failed: ${e.message || e.reason}`, e))
    },
    ...
    destroy() {
      ...
      return fsu.directories(resourcePath)
        .then(destroyDbs)
        .then(() => logger.info('Done.'))
        .catch(e => logger.error(`Database destroy failed: ${e.message || e.reason}`, e));
    },

    recreate(opts = { designDocsOnly: false }) {
      // TODO HANDLE DESTROY FAIL
      this.destroy().then(r => this.create(opts));
    }
  };
    ...
    return cloudant.db
      ...
      .catch(e => {
        if (e.statusCode === 412) {
          ...
          logger.error(`Failed to create db ${name}: ${e.message || e.reason}`, e)
        }
      });
  }
  ...
  function destroyDb(name) {
    return cloudant.db.destroy(name)
      ...
      .catch(e => logger.error(`Failed to destroy db ${name}: ${e.message || e.reason}`, e));
  }
};
