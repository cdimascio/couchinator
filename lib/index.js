...
function CouchinatorApi({ dbConfig, resourcePath, prefix, logger }) {
  ...
  const h = Commands(cloudant, resourcePath, prefix, logger);
...
function Couchinator(dbConfig) {
  ...
  const c = {
    prefix(prefix, silence) {
      if (!silence && !prefix) console.warn('called prefix(...), but no prefix specified.');
      o.prefix = prefix;
      return c;
    },
    resources(path, silence) {
      if (!silence && !path) console.warn('called path(...), but no path specified.');
      o.resourcePath = path;
      return c;
    },
